...
class AppDelegate: NSObject, NSApplicationDelegate, NSWindowDelegate {
    ...
    func runAudio() {
        ...
        udpReceiver.onSpectrumReceived = { spectrum in
            if self.wallpaperViewModel.currentWallpaper.project.general?.supportsaudioprocessing ?? false {
                let spectrumArray = spectrum.map { String($0) }.joined(separator: ",")
                let jsCode = "wallpaperAudioListener([\(spectrumArray)]);"
                
                DispatchQueue.main.async {
                    self.nsView.evaluateJavaScript(jsCode, completionHandler: nil)
                }
            }
        }
    }
...
class UDPReceiver: NSObject {
    ...
    func setupSocket() {
        ...
        addr.sin_port = CFSwapInt16HostToBig(1324)
